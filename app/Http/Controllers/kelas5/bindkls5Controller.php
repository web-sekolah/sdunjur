...
use App\Models\kelas5\bindkls5;
use App\Models\kelas5\bindkuismodel;
use App\Models\kelas5\kls5bindsubmision;
use App\Models\kelas5\kls5bindsubmitan;

class bindkls5Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls5bindsubmision::all();
            $submit = kls5bindsubmision::orderBy('id','desc')->get();
            $kuis = bindkuismodel::orderBy('id','desc')->get();
            $submited = kls5bindsubmitan::all();
            $data = bindkls5::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas5/bahasa',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas5/tambah/tambahbind');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            bindkls5::create([
                ...
                $file->move(public_path('filemateri/kelas5/bahasa'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas5/bindonesia')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = bindkls5::find($id);
        return view('kelas5/tambah/ubahbind',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas5/bindonesia')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = bindkls5::find($id);
        $matka->delete();
        return redirect('/kelas5/bindonesia')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas5/bahasa/'.$file));
    }

    public function quiz(){
        
        return view('kelas5/tambah/tambahkuisbindo');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls5bindsubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas5/bindonesia')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls5bindsubmitan::find($id);
        $submited->delete();
        return redirect('/kelas5/bindonesia')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls5bindsubmision::find($id);
        $submited->delete();
        return redirect('/kelas5/bindonesia')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls5bindsubmitan::find($id);
        return view('kelas5/tambah/editsubmisionbind',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas5/bindonesia')->with('success','Tugas anda Berhasil Diubah');
    }
}
