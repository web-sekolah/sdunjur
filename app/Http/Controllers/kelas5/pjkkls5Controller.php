...
use App\Models\kelas5\pjkkls5;
use App\Models\kelas5\pjkkuismodel;
use App\Models\kelas5\kls5pjksubsmision;
use App\Models\kelas5\kls5pjksubmitan;

class pjkkls5Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls5pjksubsmision::all();
            $submit = kls5pjksubsmision::orderBy('id','desc')->get();
            $kuis = pjkkuismodel::orderBy('id','desc')->get();
            $submited = kls5pjksubmitan::all();
            $data = pjkkls5::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas5/olahraga',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas5/tambah/tambahpjk');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            pjkkls5::create([
                ...
                $file->move(public_path('filemateri/kelas5/olahraga'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas5/pjk')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = pjkkls5::find($id);
        return view('kelas5/tambah/ubahpjk',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas5/pjk')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = pjkkls5::find($id);
        $matka->delete();
        return redirect('/kelas5/pjk')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas5/olahraga/'.$file));
    }

    public function quiz(){
        
        return view('kelas5/tambah/tambahkuispjk');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls5pjksubsmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas5/pjk')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls5pjksubmitan::find($id);
        $submited->delete();
        return redirect('/kelas5/pjk')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls5pjksubsmision::find($id);
        $submited->delete();
        return redirect('/kelas5/pjk')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls5pjksubmitan::find($id);
        return view('kelas5/tambah/editsubmisionpjk',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas5/pjk')->with('success','Tugas anda Berhasil Diubah');
    }
}
