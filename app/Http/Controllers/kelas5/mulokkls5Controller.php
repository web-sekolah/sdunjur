...
use App\Models\kelas5\mulokkls5;
use App\Models\kelas5\mulokkuismodel;
use App\Models\kelas5\kls5muloksubsmision;
use App\Models\kelas5\kls5muloksubmitan;

class mulokkls5Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls5muloksubsmision::all();
            $submit = kls5muloksubsmision::orderBy('id','desc')->get();
            $kuis = mulokkuismodel::orderBy('id','desc')->get();
            $submited = kls5muloksubmitan::all();
            $data = mulokkls5::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas5/mulok',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas5/tambah/tambahmulok');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            mulokkls5::create([
                ...
                $file->move(public_path('filemateri/kelas5/mulok'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas5/mulok')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = mulokkls5::find($id);
        return view('kelas5/tambah/ubahmulok',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas5/mulok')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = mulokkls5::find($id);
        $matka->delete();
        return redirect('/kelas5/mulok')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas5/mulok/'.$file));
    }

    public function quiz(){
        
        return view('kelas5/tambah/tambahkuismulok');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls5muloksubsmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas5/mulok')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    {   
        ...
        $submited = new kls5muloksubmitan();
        $submited->nama = $nama;
    ...
    public function destroysubmit($id){
        $submited = kls5muloksubmitan::find($id);
        $submited->delete();
        return redirect('/kelas5/mulok')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls5muloksubsmision::find($id);
        $submited->delete();
        return redirect('/kelas5/mulok')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls5muloksubmitan::find($id);
        return view('kelas5/tambah/editsubmisionmulok',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas5/mulok')->with('success','Tugas anda Berhasil Diubah');
    }
}
