...
use App\Models\kelas5\ipskls5;
use App\Models\kelas5\ipskuismodel;
use App\Models\kelas5\kls5ipssubmision;
use App\Models\kelas5\kls5ipssubmitan;

class ipskls5Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls5ipssubmision::all();
            $submit = kls5ipssubmision::orderBy('id','desc')->get();
            $kuis = ipskuismodel::orderBy('id','desc')->get();
            $submited = kls5ipssubmitan::all();
            $data = ipskls5::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas5/ips',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas5/tambah/tambahips');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            ipskls5::create([
                ...
                $file->move(public_path('filemateri/kelas5/ips'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas5/ips')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = ipskls5::find($id);
        return view('kelas5/tambah/ubahips',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas5/ips')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = ipskls5::find($id);
        $matka->delete();
        return redirect('/kelas5/ips')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas5/ips/'.$file));
    }

    public function quiz(){
        
        return view('kelas5/tambah/tambahkuisips');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls5ipssubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas5/ips')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls5ipssubmitan::find($id);
        $submited->delete();
        return redirect('/kelas5/ips')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls5ipssubmision::find($id);
        $submited->delete();
        return redirect('/kelas5/ips')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls5ipssubmitan::find($id);
        return view('kelas5/tambah/editsubmisionips',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas5/ips')->with('success','Tugas anda Berhasil Diubah');
    }
}
