...
use App\Models\kelas5\mtkkls5;
use App\Models\kelas5\mtkkuismodel;
use App\Models\kelas5\kls5mtksubmision;
use App\Models\kelas5\kls5mtksubmitan;

class mtkkls5Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls5mtksubmision::all();
            $submit = kls5mtksubmision::orderBy('id','desc')->get();
            $kuis = mtkkuismodel::orderBy('id','desc')->get();
            $submited = kls5mtksubmitan::all();
            $data = mtkkls5::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas5/matematika',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas5/tambah/tambahmtk');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            mtkkls5::create([
                ...
                $file->move(public_path('filemateri/kelas5/matematika'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas5/matematika')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = mtkkls5::find($id);
        return view('kelas5/tambah/ubahmtk',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas5/matematika')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = mtkkls5::find($id);
        $matka->delete();
        return redirect('/kelas5/matematika')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas5/matematika/'.$file));
    }

    public function quiz(){
        
        return view('kelas5/tambah/tambahkuis');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls5mtksubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas5/matematika')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls5mtksubmitan::find($id);
        $submited->delete();
        return redirect('/kelas5/matematika')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls5mtksubmision::find($id);
        $submited->delete();
        return redirect('/kelas5/matematika')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls5mtksubmitan::find($id);
        return view('kelas5/tambah/editsubmision',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas5/matematika')->with('success','Tugas anda Berhasil Diubah');
    }
}
