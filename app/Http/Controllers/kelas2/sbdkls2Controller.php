...
use App\Models\kelas2\sbdkls2;
use App\Models\kelas2\sbdkuismodel;
use App\Models\kelas2\kls2sbdsubsmision;
use App\Models\kelas2\kls2sbdsubmitan;

class sbdkls2Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls2sbdsubmision::all();
            $submit = klssbdsubsmision::orderBy('id','desc')->get();
            $kuis = sbdkuismodel::orderBy('id','desc')->get();
            $submited = kls2sbdsubmitan::all();
            $data = sbdkls2::where('Topik','LIKE','%'.$request->keyword.'%')
            ...
            $data = sbdkls2::paginate(10);
            $submit = kls2sbdsubsmision::orderBy('id','desc')->get();
            $kuis = sbdkuismodel::orderBy('id','desc')->get();
            $submited = kls2sbdsubmitan::all();
        }
        return view('kelas2/seni',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas2/tambah/tambahsbd');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            sbdkls2::create([
                ...
                $file->move(public_path('filemateri/kelas2/seni'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas2/sbd')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = sbdkls2::find($id);
        return view('kelas2/tambah/ubahsbd',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas2/sbd')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = sbdkls2::find($id);
        $matka->delete();
        return redirect('/kelas2/sbd')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas2/seni/'.$file));
    }

    public function quiz(){
        
        return view('kelas2/tambah/tambahkuissbd');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls2sbdsubsmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas2/sbd')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls2sbdsubmitan::find($id);
        $submited->delete();
        return redirect('/kelas2/sbd')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls2sbdsubsmision::find($id);
        $submited->delete();
        return redirect('/kelas2/sbd')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls2sbdsubmitan::find($id);
        return view('kelas2/tambah/editsubmisionsbd',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas2/sbd')->with('success','Tugas anda Berhasil Diubah');
    }
}
