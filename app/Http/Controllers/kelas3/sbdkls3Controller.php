...
use App\Models\kelas3\sbdkls3;
use App\Models\kelas3\sbdkuismodel;
use App\Models\kelas3\kls3sbdsubsmision;
use App\Models\kelas3\kls3sbdsubmitan;

class sbdkls3Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls3sbdsubmision::all();
            $submit = kls3sbdsubsmision::orderBy('id','desc')->get();
            $kuis = sbdkuismodel::orderBy('id','desc')->get();
            $submited = kls3sbdsubmitan::all();
            $data = sbdkls3::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas3/seni',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas3/tambah/tambahsbd');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            sbdkls3::create([
                ...
                $file->move(public_path('filemateri/kelas3/seni'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas3/sbd')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = sbdkls3::find($id);
        return view('kelas3/tambah/ubahsbd',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas3/sbd')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = sbdkls3::find($id);
        $matka->delete();
        return redirect('/kelas3/sbd')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas3/seni/'.$file));
    }

    public function quiz(){
        
        return view('kelas3/tambah/tambahkuissbd');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls3sbdsubsmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas3/sbd')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls3sbdsubmitan::find($id);
        $submited->delete();
        return redirect('/kelas3/sbd')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls3sbdsubsmision::find($id);
        $submited->delete();
        return redirect('/kelas3/sbd')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls3sbdsubmitan::find($id);
        return view('kelas3/tambah/editsubmisionsbd',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas3/sbd')->with('success','Tugas anda Berhasil Diubah');
    }
}
