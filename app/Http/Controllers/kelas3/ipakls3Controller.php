...
use App\Models\kelas3\ipakls3;
use App\Models\kelas3\ipakuismodel;
use App\Models\kelas3\kls3ipasubmision;
use App\Models\kelas3\kls3ipasubmitan;

class ipakls3Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls3ipasubmision::all();
            $submit = kls3ipasubmision::orderBy('id','desc')->get();
            $kuis = ipakuismodel::orderBy('id','desc')->get();
            $submited = kls3ipasubmitan::all();
            $data = ipakls3::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas3/ipa',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas3/tambah/tambahipa');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            ipakls3::create([
                ...
                $file->move(public_path('filemateri/kelas3/ipa'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas3/ipa')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = ipakls3::find($id);
        return view('kelas3/tambah/ubahipa',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas3/ipa')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = ipakls3::find($id);
        $matka->delete();
        return redirect('/kelas3/ipa')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas3/ipa/'.$file));
    }

    public function quiz(){
        
        return view('kelas3/tambah/tambahkuisipa');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls3ipasubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas3/ipa')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls3ipasubmitan::find($id);
        $submited->delete();
        return redirect('/kelas3/ipa')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls3ipasubmision::find($id);
        $submited->delete();
        return redirect('/kelas3/ipa')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls3ipasubmitan::find($id);
        return view('kelas3/tambah/editsubmisionipa',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas3/ipa')->with('success','Tugas anda Berhasil Diubah');
    }
}
