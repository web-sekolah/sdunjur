...
use App\Models\kelas3\ipskls3;
use App\Models\kelas3\ipskuismodel;
use App\Models\kelas3\kls3ipssubmision;
use App\Models\kelas3\kls3ipssubmitan;

class ipskls3Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls3ipssubmision::all();
            $submit = kls3ipssubmision::orderBy('id','desc')->get();
            $kuis = ipskuismodel::orderBy('id','desc')->get();
            $submited = kls3ipssubmitan::all();
            $data = ipskls3::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas3/ips',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas3/tambah/tambahips');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            ipskls3::create([
                ...
                $file->move(public_path('filemateri/kelas3/ips'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas3/ips')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = ipskls3::find($id);
        return view('kelas3/tambah/ubahips',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas3/ips')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = ipskls3::find($id);
        $matka->delete();
        return redirect('/kelas3/ips')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas3/ips/'.$file));
    }

    public function quiz(){
        
        return view('kelas3/tambah/tambahkuisips');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls3ipssubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas3/ips')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls3ipssubmitan::find($id);
        $submited->delete();
        return redirect('/kelas3/ips')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls3ipssubmision::find($id);
        $submited->delete();
        return redirect('/kelas3/ips')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls3ipssubmitan::find($id);
        return view('kelas3/tambah/editsubmisionips',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas3/ips')->with('success','Tugas anda Berhasil Diubah');
    }
}
