...
use App\Models\kelas4\mtkkls4;
use App\Models\kelas4\mtkkuismodel;
use App\Models\kelas4\kls4mtksubmision;
use App\Models\kelas4\kls4mtksubmitan;

class mtkkls4Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls4mtksubmision::all();
            $submit = kls4mtksubmision::orderBy('id','desc')->get();
            $kuis = mtkkuismodel::orderBy('id','desc')->get();
            $submited = kls4mtksubmitan::all();
            $data = mtkkls4::where('Topik','LIKE','%'.$request->keyword.'%')
            ...
            $data = mtkkls4::paginate(10);
            $submit = kls4mtksubmision::orderBy('id','desc')->get();
            $kuis = mtkkuismodel::orderBy('id','desc')->get();
            $submited = kls4mtksubmitan::all();
        }
        return view('kelas4/matematika',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas4/tambah/tambahmtk');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            mtkkls4::create([
                ...
                $file->move(public_path('filemateri/kelas4/matematika'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas4/matematika')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = mtkkls4::find($id);
        return view('kelas4/tambah/ubahmtk',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas4/matematika')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = mtkkls4::find($id);
        $matka->delete();
        return redirect('/kelas4/matematika')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas4/matematika/'.$file));
    }

    public function quiz(){
        
        return view('kelas4/tambah/tambahkuis');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls4mtksubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas4/matematika')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls4mtksubmitan::find($id);
        $submited->delete();
        return redirect('/kelas4/matematika')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls4mtksubmision::find($id);
        $submited->delete();
        return redirect('/kelas4/matematika')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls4mtksubmitan::find($id);
        return view('kelas4/tambah/editsubmision',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas4/matematika')->with('success','Tugas anda Berhasil Diubah');
    }
}
