...
use App\Models\kelas4\sbdkls4;
use App\Models\kelas4\sbdkuismodel;
use App\Models\kelas4\kls4sbdsubsmision;
use App\Models\kelas4\kls4sbdsubmitan;

class sbdkls4Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls4sbdsubmision::all();
            $submit = klssbdsubsmision::orderBy('id','desc')->get();
            $kuis = sbdkuismodel::orderBy('id','desc')->get();
            $submited = kls4sbdsubmitan::all();
            $data = sbdkls4::where('Topik','LIKE','%'.$request->keyword.'%')
        ...
        return view('kelas4/seni',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas4/tambah/tambahsbd');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            sbdkls4::create([
                ...
                $file->move(public_path('filemateri/kelas4/seni'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas4/sbd')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = sbdkls4::find($id);
        return view('kelas4/tambah/ubahsbd',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas4/sbd')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = sbdkls4::find($id);
        $matka->delete();
        return redirect('/kelas4/sbd')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas4/seni/'.$file));
    }

    public function quiz(){
        
        return view('kelas4/tambah/tambahkuissbd');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls4sbdsubsmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas4/sbd')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls4sbdsubmitan::find($id);
        $submited->delete();
        return redirect('/kelas4/sbd')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls4sbdsubsmision::find($id);
        $submited->delete();
        return redirect('/kelas4/sbd')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls4sbdsubmitan::find($id);
        return view('kelas4/tambah/editsubmisionsbd',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas4/sbd')->with('success','Tugas anda Berhasil Diubah');
    }
}
