...
use App\Models\kelas4\bindkls4;
use App\Models\kelas4\bindkuismodel;
use App\Models\kelas4\kls4bindsubmision;
use App\Models\kelas4\kls4bindsubmitan;

class bindkls4Controller extends Controller
{
    public function index(Request $request){
        if($request->has('keyword')){
            // $submit = kls4bindsubmision::all();
            $submit = kls4bindsubmision::orderBy('id','desc')->get();
            $kuis = bindkuismodel::orderBy('id','desc')->get();
            $submited = kls4bindsubmitan::all();
            $data = bindkls4::where('Topik','LIKE','%'.$request->keyword.'%')
            ...
            $data = bindkls4::paginate(10);
            $submit = kls4bindsubmision::orderBy('id','desc')->get();
            $kuis = bindkuismodel::orderBy('id','desc')->get();
            $submited = kls4bindsubmitan::all();
        }
        return view('kelas4/bahasa',['data'=>$data,'kuis'=>$kuis,'submit'=>$submit,'submited'=>$submited]);
    }

    public function create(){
        return view('kelas4/tambah/tambahbind');
    }
...
/**
    ...
    public function add(Request $request){
        ...
        if($request->file('file')==NULL){
            bindkls4::create([
                ...
                $file->move(public_path('filemateri/kelas4/bahasa'),$namafile);
                $deskripsi = $request->deskripsi;
        ...
        return redirect('/kelas4/bindonesia')->with('success','Data berhasil Ditambahkan');
    }

    public function edit($id){
        $matka = bindkls4::find($id);
        return view('kelas4/tambah/ubahbind',['matka' => $matka]);
    }
    ...
    public function update(Request $request, $id){
        if($request->file('file')==NULL){
            ...
            $matka->save();
            return redirect('/kelas4/bindonesia')->with('success','Data Berhasil Diubah');
    ...
    public function destroy($id){
        $matka = bindkls4::find($id);
        $matka->delete();
        return redirect('/kelas4/bindonesia')->with('success', 'Data Telah Dihapus!');
    }

    public function download(Request $request,$file){
        return response()->download(public_path('filemateri/kelas4/bahasa/'.$file));
    }

    public function quiz(){
        
        return view('kelas4/tambah/tambahkuisbindo');
    }
    ...
    public function addsubmission(Request $request){
        ...
        kls4bindsubmision::create([
            // dd($request->all())
            'judul' => $request->judul,
            'bataswaktu' => $request->bataswaktu,
        ]);
        return redirect('/kelas4/bindonesia')->with('success','Submission telah kamu telah terkirim');
    }
    ...
    public function destroysubmit($id){
        $submited = kls4bindsubmitan::find($id);
        $submited->delete();
        return redirect('/kelas4/bindonesia')->with('success', 'Data Telah Dihapus!');
    }
    ...
    public function destroyformsubmit($id){
        $submited = kls4bindsubmision::find($id);
        $submited->delete();
        return redirect('/kelas4/bindonesia')->with('success','Data Telah Dihapus');
    }
    ...
    public function editsubmit($id){
        $submited = kls4bindsubmitan::find($id);
        return view('kelas4/tambah/editsubmisionbind',['submited' => $submited]);
    }
    ...
    public function updatesubmit(Request $request, $id){
        ...
        $submited->save();
        return redirect('/kelas4/bindonesia')->with('success','Tugas anda Berhasil Diubah');
    }
}
